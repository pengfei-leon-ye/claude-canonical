...
CFSA self-application (per §8.3) does not extend to auditing the mechanism's procedural assumptions about Claude.ai platform behavior — e.g., the existence of `present_files` tooling, the `/mnt/user-data/outputs/` mount path, the project-settings PI editing surface, the shape of any auxiliary filesystem view, or the conversation-open snapshot model of the project knowledge base. There is no standalone platform-behavior canonical: load-bearing platform facts are captured as operating assumptions within the mechanism that consumes them (e.g., §6.1 holds the conversation-open snapshot model). When platform behavior changes, the operator revises the affected operating-assumption text directly; a dedicated platform-facts source remains reserved at [OS] §8.10 pending accumulation of multiple drifting facts.

This boundary keeps CFSA self-audit focused on **canonical content quality** (the seven dimensions in §3) rather than platform-behavior verification. A platform-behavior shift may indirectly invalidate text in CFSA (e.g., a path reference, or the §6.1 conversation-open snapshot model, becoming stale), but the corrective channel is operator-initiated revision of the affected operating-assumption text — not CFSA auditing itself for platform-behavior accuracy.
...
**Verification channel and the conversation-open snapshot model**: Hub Claude accesses canonical source content primarily through the Claude.ai project knowledge base queried via `project_knowledge_search`. The platform may additionally expose an auxiliary filesystem view (which may be empty or partially populated under GitHub-sync); it is secondary.

The project knowledge available to a conversation should be treated as a **snapshot fixed at conversation open**. Whether the `project_knowledge_search` channel reflects mid-conversation re-indexing is a Claude.ai platform behavior currently pending verification; this section holds CFSA's operating assumption on it per the §1.4 boundary. Until verified, the conservative and always-correct assumption is that a conversation's view of the canonical set does not refresh mid-conversation: a commit made during a conversation is not reliably visible to that same conversation.

**Operational rule**: post-commit verification of a canonical source's indexed state cannot be assumed to succeed within the conversation that made the commit. To verify that committed canonical changes have reached the project knowledge base, open a fresh conversation after re-indexing completes — its conversation-open snapshot reflects the post-commit state. Within the committing conversation, treat the canonical-set view as fixed at conversation open; do not wait on a mid-conversation refresh that may never arrive.

**Audit timing**: an audit run in a conversation opened before recent canonical commits audits a stale snapshot. The audit report SHALL state the conversation-open time relative to the most recent canonical commits and SHALL name the access channel used; findings derived from a snapshot that predates recent commits SHALL be flagged as "snapshot-stale" and re-verified in a fresh conversation. The operator can confirm re-indexing has completed via the indexing indicator in the Claude.ai project settings UI before opening that fresh conversation. An audit initiated in a fresh conversation opened after the most recent commits have completed re-indexing has a current snapshot and its findings are definitive.
