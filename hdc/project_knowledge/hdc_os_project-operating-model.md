...
Seven premises hold across all four work categories in this project. They are the irreducible foundation for the project's design choices and the load-bearing anchor for anti-mimicry framing in downstream canonical sources.
...
- Development Track inherits canonical constraints via the **decoupled-reference model** (per [REF] Hub-CD-CC Architecture §5.4.4 post-Phase-3): CC's CLAUDE.md hierarchy and CC's own canonical layer at `<repo-root>/.claude/canonical/` reference Hub constitutional residues by name without inlining their content. The inheritance scope currently includes Hub-side constitutional residues plus CC substantive canonical:
  - **Hub constitutional residue at Hub PK**:
    ...
    - [MECH] Development Track Workflow (constitutional residue + Hub-internal substantive — TK chain identity, Hub-authored TK-01 / TK-02-main / TK-12, transitions; TK-02.3 UX-spec synthesis (S1) + TK-03 acceptance / intent authoring (S2) are CC-side firewalled substantive per [REF] §5.4.4)
    - [MECH] CI/CD Milestone Policy (constitutional residue — M0–M4 ladder identity, Test Evidence Report schema, evidence parity)
  ...
  - **CC substantive canonical at CC's own canonical layer** (migrated in Phase 3 per [OS] §0.1.5 Premise 5):
    - CC substantive CCAR (subagent roster, named context scopes, repository path patterns, skill catalog, domain lifecycle and Pact contract testing specifics)
    ...
    - CC substantive DTW (TK-04~TK-12 execution mechanics, CC-side transitions)
...
- Development Track runtime capabilities include:
  - the subagent roster under `.claude/agents/` with bias firewall enforced by `.claude/config/context-scopes.yaml` (membership / count owned by CC substantive CCAR as SOT — not pinned here)
...
| P-28 (Conversion Spec §2 + §3.8 ↔ TDD §4.{feature-slug}.Module-Decomposition + UX Design Spec §2 — covering both §2A phase-level categories and §2B per-feature categories) | 3 — Content-category extraction interlock | Conversion Spec's extraction logic depends on UX Design Spec's content-category structural taxonomy (across both §2A and §2B) referenced by semantic name |
...
| P-09 | [MECH] DTW §4.0 ↔ [MECH] CI/CD §2.6 (per-unit-type milestone profile) | SR | C2C | new unit_type added, milestone path modified, or scope variation revised |
...
| P-28 | [TPL] Conversion Spec §2 + §3.8 ↔ [TPL] TDD §4.{feature-slug}.Module-Decomposition + [TPL] UX Design Spec §2 (§2A phase-level + §2B per-feature) | SP | C2C | TDD per-feature structure changes (module decomposition restructured) or UX Design Spec content-category structure changes (§2A or §2B categories renamed, merged, split, or rescoped, or the §2A↔§2B granularity split itself revised) |
| P-29 | [TPL] Intent-Acceptance §2.3 + §3.9 ↔ [TPL] UX Design Spec §2 (§2A phase-level + §2B per-feature) | SP | C2C | UX Design Spec content-category structural organization changes (§2A or §2B categories renamed, merged, split, or rescoped, or the §2A↔§2B granularity split itself revised) — this is the producer-side change that requires re-verifying the consumer-side Writing Standard fields |
...
| P-54 | [RULE] DSG §5.2.4 ↔ [TPL] UX Design Spec §2A.6 + §2B.4 | SP | C2C | the DSG §5.2.4 reuse-scope criterion (cross-cutting vs feature-specific boundary, incl. recognized structural seams) changes, or the UX Design Spec §2A.6 cross-cutting index / §2B.4 scope-declaration classification semantics change |

**Pairings updated in Phase 3** (§ references in active pairings updated to match constitutional residue § numbers):

- **P-09** § reference: `[MECH] CI/CD §2.0` → `[MECH] CI/CD §2.6` (the per-unit-type milestone profile is at §2.6 in the constitutional residue)
...
- **P-09** — DTW §4.0 catalog and CI/CD §2.6 profile hold the same data viewed from two angles (per-unit-type task path vs per-unit-type milestone subset). When either side changes, the other must update in the same revision; divergence creates immediate operational ambiguity for Hub Claude scheduling and Claude Code execution.
...
- **P-28** — Module-driven slicing (Conversion Spec §2) depends on TDD producing MECE per-feature module decomposition; UX brief extraction (§3.8) depends on CC-authored UX Design Spec instances at both granularities (phase-level + per-feature; synthesized in the CC firewalled S1 session — the relocated TK-02.3, per [REF] Hub-CD-CC Architecture §5.4.4) being present when Tier 1 is involved. When TDD per-feature sub-section structure changes, the module-side extraction rules must be re-verified; when UX Design Spec content-category structure changes (§2A phase-level categories renamed/merged/split, §2B per-feature categories renamed/merged/split, or the §2A↔§2B granularity split itself revised), the UX-side extraction rules in Conversion Spec §3.8 must be re-verified.
- **P-29** — The Intent-Acceptance Writing Standard references UX Design Spec instance content categories by semantic name across both granularities — phase-level §2A (platform shell, shared vocabulary, cross-feature touchpoints, phase decisions, VR naming convention, cross-cutting additive index) and per-feature §2B (Affected Tier 1 scope, layout pattern selection, components referenced, new components or tokens, accessibility call-outs, internationalization call-outs, VR anchors, responsive/motion) — rather than by § number. When UX Design Spec content-category structural organization changes (§2A or §2B categories renamed, merged, split, or rescoped, or the §2A↔§2B granularity split itself revised), the semantic references in Writing Standard §2.3 UX brief sub-section field rules and §3.9 accessibility_expectations source mapping must be re-verified.
...
- **P-54** — DSG §5.2.4 defines the Tier B reuse-scope criterion (cross-cutting vs feature-specific, including the recognized structural seams such as `ObjectPage` facet-body injection); UX Design Spec §2A.6 (cross-cutting additive index) and §2B.4 (per-feature scope declaration) are where that classification is applied at UX-spec synthesis time (the CC firewalled S1 session — the relocated TK-02.3, per [REF] Hub-CD-CC Architecture §5.4.4). When the §5.2.4 criterion changes (boundary test or recognized seams), the §2A.6/§2B.4 scope-declaration guidance must be re-verified so the CC `ux-spec-synthesizer` classifies against the current criterion; when the §2A.6/§2B.4 classification semantics are restructured, §5.2.4 must be re-verified for continued alignment. Same-period (not same-revision): a stale criterion-vs-implementation gap misclassifies a Tier B onto the wrong registration surface (instance §4 + §2A.6 vs feature §2B.4) and compounds as drift rather than breaking a running flow immediately.
...
- **P-05**: RETIRED. AI-dev side no longer produces release tags; handoff tag namespace `handoff/{app-slug}/{YYYY-MM-DD}` is the sole canonical tag namespace.
...
|---|---|---|
| `[RULE] Claude Code Architecture Rules` (CCAR) — **constitutional residue** | Static structure (constitutional) | Three-tier architecture identity (§1), Tier 2 thinning rule (§2), permission decision placement principle (§3), CLAUDE.md hierarchy existence pointer (§4), subagent topology existence (§5), high-level monorepo structure (§6), cross-workspace anti-drift signals (§7). **CC-side substantive**: the subagent roster, named context scopes, repository path patterns, skill catalog, domain lifecycle and contract-testing operational specifics (at CC substantive CCAR canonical) |
...
| `[MECH] Development Track Workflow` (DTW) — **constitutional residue + Hub-internal substantive** | Task-level orchestration (constitutional) + Hub-authored task content (Hub-internal substantive) | TK-01 through TK-12 identity (§4); full Hub-authored substantive content for TK-01 / TK-02 main (the TDD, Hub-internal substantive); TK-12 M4 operator merge-decision terminal gate (Hub-internal substantive); unit_type catalog (§4.0, constitutional interface); transition mechanism catalog (§5); human intervention budget (§6); failure routing matrix (§7); cross-workspace anti-drift (§8); Hub Claude soft compliance trigger phrases (§9, Hub-internal substantive). **CC-side substantive**: TK-02.3 UX-spec synthesis (S1) and TK-03 acceptance / intent / test-plan authoring (S2) — authored in CC sessions firewalled from the implementing session (S3), per [REF] Hub-CD-CC Architecture §5.4.4 — plus TK-04 through TK-12 execution mechanics including specific subagent invocations and tool commands (at CC substantive DTW canonical) |
| `[MECH] CI/CD Milestone Policy` — **constitutional residue** | Lifecycle gating (constitutional) | M0–M4 ladder identity (§2.1-§2.5), per-unit-type milestone profile interface (§2.6), Test Evidence Report schema (§3), required artifact output gates (§4), multi-node evidence parity invariant (§1.2), cross-workspace anti-drift (§5). **CC-side substantive**: gate criteria per M-N, tooling baseline, accessibility thresholds, slice-size advisory, stuck recovery, code review tool operational specifics, performance test scope (at CC substantive CI/CD canonical) |
...
- `[MECH] Dev-Loopback Mode` → CC substantive Dev-Loopback Mode canonical (single-command startup, fixture content, placeholder pattern, env switch gate, walking-skeleton M4 acceptance assertions, HANDOFF.md)
...
**Attention-budget criterion (AI-consumption-only)**: the thresholds above are structural and operator-maintenance-oriented (pairing fan-out, source count, line count — proxies for the operator's paired-update and working-memory burden). A distinct, additional trigger fires on aggregate **model**-attention load: when total retrieved-canonical volume under normal RAG patterns risks pushing load-bearing / constitutional rules out of the model's reliable-attention zones — lost-in-the-middle / context rot persist regardless of nominal context-window size, so usable attention scales sub-linearly with raw window. This criterion is evaluated at the set / aggregate level (the §3-dimensional per-source audit in [MECH] Canonical File Self-Audit cannot observe a set-level property), and measures the model's attention budget, never the operator's reading or maintenance burden. When it fires, surface as an anti-drift signal per §12 and apply the same structured re-architecture evaluation below, with SOT consolidation and pairing deprecation as the primary levers for reducing aggregate retrieved volume. **Measurement proxy** (heuristic, not a bright line): "risks pushing rules out of attention" is assessed by the retrieval-position and surfacing-reliability of load-bearing / constitutional rules under normal RAG patterns — whether such rules reliably appear in retrieved context and land outside the lost-in-the-middle zone — rather than by raw token count alone. Absent a cheap instrument, a conservative operator-judgment estimate suffices; the proxy exists to make the criterion testable, not to mandate metric tooling.
...
| `[REF]` | `Reference Catalog` | Used across Cat 2, Cat 4, and Cross-category reference sources |
...
| `[MECH]` (governance-focused) | `Governance Mechanism Specification` | e.g., CI/CD Milestone Policy, Canonical File Self-Audit, Sign-Off Cleanup Policy |
...
| Cross-node Codex invocation anti-pattern | [RULE] WT constitutional residue + CC substantive Codex Plugin Usage canonical (co-owned post-Phase-3) | WT residue §6 + CC-side Codex | DTW §8 (TK-04 / TK-11 trigger-time view) |
