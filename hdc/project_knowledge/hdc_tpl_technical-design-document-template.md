...
- **Role**: Reusable template for producing a phase-level Technical Design Document (TDD) that captures, per phase per app, foundational architecture (Phase 1) or architecture deltas (Phase N ≥ 2), cross-feature concerns (NFR baselines, security, observability, deployment), walking skeleton scope (Phase 1 only — full canonical specification at §3 referencing CC substantive Workspace Topology canonical (walking-skeleton 6-output set) + [RULE] WT residue §3 (ordering rule) and [MECH] CI/CD Milestone Policy constitutional residue §2.6 (per-unit-type milestone profile interface)), and per-feature engineering spec for every feature introduced in that phase. UX strategy content is intentionally NOT part of the per-feature engineering spec; it lives in UX Design Spec instances CC-authored in the firewalled UX-spec synthesis session (S1, the `ux-spec-synthesizer`, the relocated TK-02 Step 2.3, per `[TPL] UX Design Spec`) at two granularities — phase-level (cross-cutting UX) + per-feature (feature-scoped UX) — when any feature in the phase touches Tier 1.
...
- **Relationship to [PRIN] People Experience Design Principles**: Applied via UX Design Spec instances (CC-authored in the firewalled S1 UX-spec synthesis session per `[TPL] UX Design Spec`, at phase-level and per-feature granularities) when any feature touches Tier 1; the TDD does not embed UX strategy content
- **Relationship to [REF] Hub-CD-CC Architecture**: TDD is Hub-authored (upstream specification main body, the coherence anchor per §4.3); when any feature's `Header.tier_1_involved: true`, the TDD's per-feature declaration plus the phase-level scope triggers UX Design Spec instance authoring at two granularities (phase-level + per-feature), performed CC-internally in the firewalled S1 UX-spec synthesis session (`ux-spec-synthesizer`, the relocated TK-02 Step 2.3); the resulting instance is part of CC's detailed spec consumed alongside this TDD when slice implementation begins
...
- **Relationship to [MECH] CI/CD Milestone Policy**: The Milestone Policy constitutional residue owns the M0–M4 gate ladder and the per-unit-type milestone profile interface (§2.6, referenced at §3), but not slice-size limits or accessibility thresholds — both are CC-side substantive. Per-feature module decomposition slice-size advisory soft upper limits are checked at §5.4 against CC substantive CI/CD Milestone Policy canonical (slice-size advisory), per the same decoupled-reference model this file uses elsewhere; phase-level a11y testing strategy (§2.2.7) aligns with accessibility thresholds that are likewise CC substantive, and no accessibility gate exists at any milestone (constitutional invariant per [RULE] Design System Governance §6 stance), so the strategy is threshold-aligned rather than gate-bound.
- **Relationship to [RULE] Design System Governance**: TDD per-feature `§4.{feature-slug}.Header.tier_1_involved` declaration triggers UX Design Spec instance authoring at phase-level and per-feature granularities, both governed by DSG. TDD itself does not author DS-coupled UX content; it declares scope and leaves authoring to CC's firewalled S1 UX-spec synthesis session (`ux-spec-synthesizer`), which synthesizes from the DS mirror + PRD/TDD and, when a feature trips the visual-novelty trigger, an on-demand CD-produced design file as visual reference per the two-way distribution model in DSG §1.1.
- **Relationship to adjacent [TPL] sources**:
  - Downstream of `[TPL] PRD / Prototype / MVP Spec Template` (1:1 phase-level pairing)
  - Paired with `[TPL] UX Design Spec` — per-feature `Header.tier_1_involved: true` triggers a per-feature UX Design Spec instance for that feature (synthesized CC-internally in the firewalled S1 session), and when any feature in the phase has `tier_1_involved=true` the phase-level UX Design Spec instance is produced as well; this template does not embed UX strategy content. TDD §2.2.5 Integration boundaries (interface-level contracts with external systems) is paired-but-non-overlapping with phase-level UX Design Spec §2A.3 Cross-feature touchpoints (interaction-level contracts between in-app features) — see §2.2.5 boundary note below
...
The TDD is the canonical technical reference for **a single phase of one app**. One phase produces one TDD. The TDD answers the "how" the paired phase PRD deliberately leaves open. Human-readable; primarily reviewed by you; referenced by the CC-side detailed-spec authoring sessions (the relocated TK-02.3 UX-spec synthesis / TK-03 acceptance, firewalled S1/S2) during per-slice extraction; consumed by the implementing CC session during M0 entry self-check (TK-04) and TK-05 onward.
...
**Primary**: You (architect / decision owner) + the CC-side detailed-spec authoring session (reference during the relocated TK-03 / S2).
...
- **UX Design Spec owns**: per-feature UX coverage (affected Tier 1 scope, components from DS instance, new-component additive update plans, layout patterns, accessibility call-outs, i18n/RTL call-outs, visual regression anchors, responsive/motion expectations) — CC-authored in the firewalled S1 UX-spec synthesis session per `[TPL] UX Design Spec`; independently read by the `ux-spec-cross-checker` (S1, propose-not-author) against the synthesis source; consumed during slice authoring
...
- UX and accessibility scope for intent.md UX brief and acceptance a11y expectations — from the feature's UX Design Spec instance (CC-authored in the firewalled S1 UX-spec synthesis session per `[TPL] UX Design Spec`)
...
**Boundary with phase-level UX Design Spec §2A.3 (Cross-feature touchpoints)**: This sub-section covers **external** integration boundaries (system-to-system contracts crossing the app's boundary). **Inter-feature** contracts within the app — where one feature's UI action triggers another feature's flow (CTA jumps, state propagation, masked-data visibility, change-set → review-inbox handoff, etc.) — belong to the phase-level UX Design Spec instance §2A.3 Cross-feature touchpoints at the interaction-level, with per-feature `§4.{feature-slug}.API-Contracts` carrying any interface-level (API / event / data) contract the touchpoint also requires. A single inter-feature touchpoint may have both an interaction side (UX Spec §2A.3) and an interface side (TDD §4.{feature-slug}.API-Contracts); the two are paired but non-overlapping.
...
**Purpose**: Define the thinnest end-to-end vertical slice that proves the foundational architecture (§1) and cross-feature baselines (§2) work together end-to-end through the AI-dev CI/CD pipeline (M0 → M4 merge to `main`), before any customer-meaningful feature unit in Phase 1 begins execution. The walking skeleton is itself a node-level work unit (`unit_type: walking_skeleton`) per [RULE] Workspace Topology constitutional residue §4 (node-assignment interface contract) and [MECH] Development Track Workflow §4.0.2; the unit consists of exactly one slice that runs the full M0 → M4 milestone chain. Walking skeleton is **production-grade code, not throwaway prototype** — anchored on Cockburn 2004 *Crystal Clear* and Freeman & Pryce 2009 *Growing Object-Oriented Software, Guided by Tests*, the walking skeleton is merged to `main` via M4 in the first or second sprint of Phase 1 and is the canonical receiving-company-side starting point for subsequent staging / production deploy post-handoff. The operational reason it must be production-grade is that it has to traverse the app's real M0 → M4 AI-dev CI/CD pipeline — build, test gates, dev-loopback acceptance assertions, code review, and operator-gated merge — so that a successful M4 pass empirically asserts the pipeline is established; a throwaway prototype that bypasses those gates would assert nothing.
...
> Walking skeleton scope establishes the thinnest end-to-end vertical slice that proves the foundational architecture (§1) and cross-feature baselines (§2) work together before Phase 1 feature units begin. The walking skeleton has near-zero customer-visible value but completes the full M0 → M4 AI-dev CI/CD chain (merge to `main`), empirically asserting that the CI/CD pipeline is established for this app. The walking skeleton is production-grade code per Cockburn 2004 / Freeman & Pryce 2009 and is merged to `main` in the first or second sprint of Phase 1; staging / production deploy from `main` is the receiving company's CI/CD scope post-handoff per [MECH] Application Lifecycle Handoff §0.2.
...
- **CI/CD pipeline establishment**: the walking skeleton's M4 milestone pass (`status: merged` on `main` plus dev-loopback acceptance assertions) empirically asserts the AI-dev CI/CD pipeline is established for this app through merge to `main`. The author does not write per-step CI/CD content (CI/CD is owned by [MECH] CI/CD Milestone Policy); the assertion is implicit in the milestone choreography. The AI-dev environment does not produce staging or production deploys — both are the receiving company's CI/CD responsibility after handoff per [MECH] Application Lifecycle Handoff §0.2
...
The walking_skeleton unit runs the full M0 → M1 → M2 → M3 → M4 milestone chain per the per-unit-type milestone profile in [MECH] CI/CD Milestone Policy constitutional residue §2.6 (per-unit-type milestone profile interface). Per-milestone semantics, automated actions, user-gate requirements, and Codex fire conditions are owned by that source.
...
- **M4 acceptance (terminal)**: state the dev-loopback acceptance assertions the walking skeleton must pass at M4 pre-merge (per CC substantive Dev-Loopback Mode canonical). The successful M4 merge of the walking skeleton (`status: merged` on `main` with dev-loopback assertions passing) constitutes the empirical assertion that the AI-dev CI/CD pipeline is established for this app through merge to `main`. Staging and production deploy are out of AI-dev scope per [MECH] Application Lifecycle Handoff §0.2.
...
- TK-03 per-slice acceptance/intent extraction (CC-side, authored in the firewalled S2 session ⊥ the implementing session, per `[REF] Hub-CD-CC Architecture` §4.3) can begin for any feature whose §4 sub-section is stable
...
After module decomposition, verify that the resulting slices respect the soft upper limits declared in CC substantive CI/CD Milestone Policy canonical (slice-size advisory):
...
> **v0 assumption — to be calibrated per CC substantive CI/CD Milestone Policy canonical (slice-size advisory)**: The 10-files / 500-LOC heuristics are starting points; first-feature lessons-harvest may revise them.
...
- Estimated slice scope (file count + net LOC) per the §5.4 slice-size advisory check; flagged-as-oversized when the estimate exceeds CC substantive CI/CD Milestone Policy canonical (slice-size advisory) limits
...
A `UX-Strategy` sub-section is intentionally **not** part of the per-feature TDD content. When `Header.tier_1_involved` is true, the feature's UX coverage lives in a separate UX Design Spec instance CC-authored in the firewalled S1 UX-spec synthesis session per `[TPL] UX Design Spec` — outside the TDD body. See §0.4 boundary for ownership clarity.
...
| Per-feature UX coverage (affected Tier 1 scope, components from DS instance, new-asset additive update plans, layout patterns, slice-specific a11y, i18n/RTL call-outs, visual regression anchors, responsive/motion expectations) | UX Design Spec instance (CC-authored in the firewalled S1 UX-spec synthesis session per `[TPL] UX Design Spec`); independently read by the `ux-spec-cross-checker` (S1, propose-not-author); consumed at slice authoring |
...
- Phase PRD new user flow within a feature → that feature's UX Design Spec instance updated (CC-side, re-synthesized in the firewalled S1 session); TDD `§4.{feature-slug}.Header.tier_1_involved` flag re-verified if user flow changes Tier 1 involvement
...
The TDD per-feature `§4.{feature-slug}.Header.tier_1_involved: true` declaration is what binds this TDD to the DSG ecosystem. The TDD itself does not author UX strategy content; the UX Design Spec instance (CC-authored in the firewalled S1 UX-spec synthesis session per `[TPL] UX Design Spec`) carries that content. When either changes:
- Design System Governance additive update (new component / token added to instance) → in-flight features may reference the new asset by authoring it into the per-feature UX Design Spec instance §2B.3, no TDD modification required
...
- per-feature UX Design Spec instance §2B.4 new-component / new-token plan → triggers DS instance update flow per DSG §12 at the originating feature's merge-to-main milestone; TDD `§4.{feature-slug}` is unchanged by this flow
...
- tier_1_involved: <true|false>   [if true, a UX Design Spec instance is CC-authored in the firewalled S1 UX-spec synthesis session per [TPL] UX Design Spec — not embedded in this TDD]
...
24. **For each feature with `Header.tier_1_involved: true`**: a UX Design Spec instance has been CC-authored in the firewalled S1 UX-spec synthesis session per `[TPL] UX Design Spec` and independently read by the `ux-spec-cross-checker` (S1, propose-not-author) against that template's §3 reviewer checklist; instance is paired with this TDD's `§4.{feature-slug}` by feature-slug match
25. **For each feature declaring new components or tokens in its per-feature UX Design Spec instance §2B.4**: additive-update plan is complete and defensible per [RULE] DSG §12 change content structure
26. **For each feature declaring slice-specific a11y considerations in its per-feature UX Design Spec instance §2B.5**: they are written as concrete concerns (not generic restatements of DSG §6.1 recommendations), with optional verification path noted (manual smoke test, on-demand SK-W audit, or eslint-plugin-jsx-a11y warning)
