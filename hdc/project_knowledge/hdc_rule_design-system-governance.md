...
14. (If the update touches §4 tokens) token lifecycle states are valid — every deprecation names a live successor (Rule 4.3.2), no token is `removed` while a merged slice still references it (Rule 4.3.4), and any rename followed the add→deprecate→remove sequence (Rule 4.3.5); **and** no `deprecated` token is used as a **live binding** (component state→token mapping, composition / token-mapping / accessibility prose, or glossary "Binds …" line) anywhere except its own IT3/§3.2 deprecation/tombstone row — the deprecate-with-successor migration-residue check (mechanically pre-screened at §12.7(e))
