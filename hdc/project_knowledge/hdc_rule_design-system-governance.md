...
- **Relationship to [OS]**: Serves the Specify loop by establishing the design-system governance framework that downstream specification artifacts (PRD, TDD, intent, acceptance, and the UX Design Spec — synthesized in a firewalled CC session, S1) must respect; subject to [OS] §8.5 paired-update consistency. Grounded in [OS] §0.1 project-level operating premises and [OS] §0.2 Cat 4 role anchor.
...
- **Relationship to [REF] Hub-CD-CC Architecture**: This rule's SOT-distribution declarations in §1.1 align to §5.2 two-way distribution model (CD = SOT / CC = code-time mirror). CD is the instance SOT; CC carries a read-only mirror at `specs/design-system.md` for code-time consumption. Hub holds no DS instance copy. CD app-level visual authoring is default-retired (§1.1 / §13.3): app-level screens default to CC-direct Arco-React with no design file; a CD design file enters only via the on-demand re-entry path (genuine visual novelty), consumed by CC as a visual reference per §13.3. When the architecture's distribution model changes substantively, §1.1 of this rule is re-verified.
- **Relationship to [RULE] Claude Code Architecture Rules**: Constrains Tier 1 (React) work exclusively; Tier 2 and Tier 3 are out of scope. The CC mirror of the DS instance is a project-level singleton at the monorepo root, not under any `apps/{app-slug}/` directory; downstream feature artifacts that reference it (the UX Design Spec synthesized in a firewalled CC session (S1) per `[TPL] UX Design Spec`, intent.md UX brief and test-plan.yaml accessibility cases authored in a firewalled CC session (S2, the relocated TK-03)) are app-scoped under `apps/{app-slug}/specs/...`. The repository-layout and code-scope substantive detail (the monorepo-root placement rule and the `business_rules_only` scope allow list that includes the CC mirror as `specs/design-system.md`) is owned by the CC-side substantive Claude Code Architecture Rules canonical.
- **Relationship to [MECH] Development Track Workflow**: The DS instance and the CC mirror are established at workspace inception per [RULE] Workspace Topology constitutional residue §5 (workspace inception governance) via initial CD authoring + DS markdown export to the CC mirror location (after the §15 export review per §12). UX Design Spec synthesis runs in a firewalled CC session (S1) per §13.3; app-level screens default to CC-direct Arco-React with no CD design file. A CD design file enters only via the on-demand re-entry path (genuine visual novelty), and is then consumed by CC as a visual reference informing code, not as a standing per-phase spec-authoring input. CC mirror is referenced by TK-04 (M0 entry, per-slice spec consumption) and TK-05+ (code writing, M3 visual review). Additive updates to the instance authored in CD propagate to the CC mirror via the §12 sync mechanism in this rule.
- **Relationship to [MECH] Cross-Tool Workflow Handoff**: §2 (Hub ↔ CD path) carries DS-related discussions and DSG governance text into CD as a read-only input for DS instance authoring (per §12.1), and the CD-generated DS markdown export transferred to Hub for the §15 export conformance review; §3 (Hub ↔ CC path) carries DSG governance text into CC and transfers the reviewed DS markdown export to the CC mirror; the on-demand app-level design-file re-entry path is a CC ↔ CD flow (CC pushes the feature's PRD/TDD text into the CD project's `uploads/` via the DesignSync write-half → operator designs in CD → CC pulls the design file back via the read-half → CC consumes it as a visual reference), not a Hub-mediated transfer. The export-review cadence and triggers are owned by §12 of this rule.
...
- **Relationship to custom skills**:
  ...
  - Consumed in the firewalled CC UX-spec synthesis session (S1) and, on the on-demand re-entry path, against a CD design file held as a visual reference (Hub/CC hold no DS mirror); the consumption discipline is described in §13.3
...
- Synthesizing a UX Design Spec instance in the firewalled CC session (S1) — phase-level (cross-cutting content) or per-feature (feature-scoped content), by default from PRD/TDD design intent; this rule declares the governance baseline for what design content the instances must respect
- Consuming a CD design file on the on-demand re-entry path — when a feature carries genuine visual novelty (proxied by "needs a new design token / new visual language"), CC pushes its PRD/TDD text into the CD project's `uploads/`, the operator designs in CD, and CC pulls the design file back as a visual reference for synthesis and code — per §13.3
...
CD app-level visual authoring is **default-retired**: app-level screens default to CC producing Arco-React directly with no CD design file, and UX Design Spec synthesis runs in a firewalled CC session (S1, §13.3) rather than from a per-phase design file. A CD design file is consumed only on the on-demand re-entry path (genuine visual novelty, §13.3), as a visual reference — not a DS instance copy. The export conformance review (§12.3) remains the Hub-side touchpoint with DS instance content, performed transiently on the export rather than via a persisted mirror. (The two-way SOT distribution above — CD = SOT, CC = code-time mirror, Hub holds no DS instance copy — is unchanged by the app-level retire-default; it governs the meta-DS vocabulary, not app-level screen visuals.)

Update categories:
- **Additive updates** (new component added, new token added, i18n scope expanded, new layout pattern added): authored in CD per a feature's UX needs (the additive plan is captured as the corresponding feature's per-feature UX Design Spec instance §2B.4 New-Components-Or-Tokens entry, synthesized in the firewalled CC session (S1); a new-token entry is itself the signal that trips the on-demand CD re-entry trigger per §13.3). Once approved per §12, CD finalizes the change; CD regenerates the DS markdown export; the export is reviewed Hub-side against §15 per §12.3; on a passing review the **CC mirror** (`specs/design-system.md`) is re-synced, typically at the feature's merge-to-main milestone (see [MECH] Development Track Workflow TK-12 M4 gate).
...
| CD-authored design file (**on-demand only** — default-retired; produced only when a feature carries genuine visual novelty, proxied by a new design token / new visual language) | CD workspace; on the on-demand re-entry path, pulled into CC as a visual reference (§13.3) | Feature/phase-scoped to the visual-novelty trigger | CD-native design file: hi-fi mockups, prototypes, wireframes, component callouts, interaction flows with embedded textual annotations. Default app-level path produces **no** design file (CC generates Arco-React directly); when re-entry is triggered, CC consumes the design file as a visual reference informing synthesis and code |
| **Phase-level UX Design Spec instance per `[TPL] UX Design Spec` (synthesized in the firewalled CC session S1)** | `apps/{app-slug}/specs/ux-design-spec/phase-{N}.md` | Phase-level | Platform shell + IA + personas, shared visual vocabulary, cross-feature touchpoints, phase-level horizontal design decisions, visual regression naming convention, cross-cutting new components/tokens additive index; written in markdown for AI-RAG consumption |
| **Per-feature UX Design Spec instance per `[TPL] UX Design Spec` (synthesized in the firewalled CC session S1)** | `apps/{app-slug}/specs/ux-design-spec/{feature-slug}.md` | Feature-level | Affected Tier 1 scope, components from DS instance referenced by this feature, new components introduced (with §12 update plan; scope: feature-specific or cross-cutting first-owner), feature-specific a11y notes, mobile/PC scope; written in markdown for AI-RAG consumption |
| `apps/{app-slug}/specs/intent/{slice-id}.md` UX brief section | Authored in the firewalled CC session (S2, the relocated TK-03), from the UX Design Spec instance (+ a CD design file as visual reference only on the on-demand re-entry path) | Slice-level | Screen list, interaction highlights, i18n/a11y call-outs specific to the slice |
| `apps/{app-slug}/specs/acceptance/{slice-id}.yaml` | Authored in the firewalled CC session (S2) | Slice-level | Machine-readable acceptance scenarios including a11y expectations |
| `apps/{app-slug}/specs/test-plan/{slice-id}.yaml` | Authored in the firewalled CC session (S2) | Slice-level | Test cases including `test_type: accessibility` |
...
**Rule 4.3.6 — Successor rebinding must fully propagate (no migration residue)**: When a token moves `active → deprecated` under Rule 4.3.2 and its semantic is rebound to the successor, **every authoritative binding of that semantic in the instance** — component state→token mappings, composition / token-mapping / accessibility-posture prose, and glossary "Binds …" lines — MUST name the successor, not the deprecated token. The deprecated token MAY appear only in (a) its own IT3 / §3.2 deprecation-or-tombstone row and (b) deprecation-describing prose (overview, change-category, change log). A deprecated token left naming itself as a live binding elsewhere is **migration residue** — an authoritative mapping pointing at a token the same instance has retired, leaving the instance internally self-contradictory. This is distinct from the Rule 4.3.3 alias: 4.3.3 lets *not-yet-migrated downstream slices* keep rendering through the alias during the transition window (a consumer-side grace); 4.3.6 requires the **instance's own** bindings to be fully migrated at rebinding time, not left citing the retired token. Mechanically pre-screened at §12.7(e); confirmed at §15 item 14.
...
- The project owner
- The firewalled CC spec-authoring sessions (the proposal originates when synthesizing a phase-level or per-feature UX Design Spec instance (S1) reveals a gap in current DS, or when intent/acceptance authoring (S2, the relocated TK-03) hits an interface need that cannot be met); the change is then authored in CD where the instance lives
- Claude Code (CC) may **propose** a DS change when code generation reveals a gap (a needed component / token / pattern absent from the mirror) — surfaced as a `design-debt` issue-log entry and routed to the originating feature's §2B.4 path via the operator-mediated CC → Hub → CD flow ([REF] Hub-CD-CC Architecture §9.3). **Propose-not-write**: CC's role ends at surfacing the gap. CC does **not** author the CD-side DS instance content, and in particular does **not** use the DesignSync write-half (`write_files` / `delete_files`) against the DS SOT. Authoring stays CD-side; ratification stays the §12.3 process + §15 review.

## 12.2 Change categories

- **Additive** (new component, new token, new locale, new layout pattern, new icon): proposed in the originating feature's per-feature UX Design Spec instance §2B.4 New-Components-Or-Tokens, synthesized in the firewalled CC session (S1) — a new-token entry here is also the signal that trips the on-demand CD re-entry trigger (§13.3); cross-cutting additives (used by multiple features in the phase) are additionally indexed in the phase-level UX Design Spec instance §2A.6 cross-cutting additive index, which cross-references the originating feature's §2B.4 entry holding the authoritative plan content; the proposal travels via operator-mediated transfer to CD per [MECH] Cross-Tool Workflow Handoff §2.1 for CD-side instance authoring; the change merges into the DS instance (CD-side) when the originating feature's slice merges to main (see [MECH] Development Track Workflow TK-12); the CC mirror is re-synced from the reviewed DS markdown export after the CD-side change is finalized.
...
1. Proposer surfaces the need (in the firewalled CC UX-spec synthesis session (S1), in the firewalled CC intent/acceptance authoring session (S2, the relocated TK-03), or directly in a CD authoring session)
2. For additive changes: the change plan is authored as a §2B.4 entry in the originating feature's per-feature UX Design Spec instance (synthesized in the firewalled CC session S1 at `apps/{app-slug}/specs/ux-design-spec/{feature-slug}.md`); if the additive is cross-cutting (used by multiple features in the phase), an index entry is additionally added in the phase-level UX Design Spec instance §2A.6 cross-referencing the §2B.4 entry. For breaking changes: change is drafted in CD-native form as a standalone change file (CD-internal location)
3. **Governance-rule review** (against §3-§11 of this rule):
   - For additive: conformance with §3-§11 is checked while synthesizing the per-feature UX Design Spec instance (the §2B.4 entry) and the phase-level instance (the §2A.6 index when cross-cutting) in the firewalled CC session (S1); operator signs off
   - For breaking: a separate review gate is convened; conformance against §3-§11 of this rule is checked
...
- **Rationale**: the business or design need motivating the change, traceable to the triggering feature's PRD or the per-feature UX Design Spec instance §2B.4 (synthesized in the firewalled CC session S1) when applicable (cross-cutting additives additionally reference the phase-level instance §2A.6 index entry)
...
- **CC authoring the CD-side DS SOT** — CC writing DS instance content directly into a CD design-system project (e.g., via the DesignSync `write_files` / `delete_files` write-half), bypassing the §12.1 propose path and §12.3 / §15 ratification. CC surfaces DS gaps (propose-not-write, §12.1); it never writes the ratified DS SOT. (Distinct from the CC-mirror-read-only rule above: that governs the mirror, this governs the SOT.)
...
**Structural pre-check (gates the §12.3 conformance review)**: Before the §12.3 semantic conformance review consumes reviewer attention, the export passes a lightweight **structural** validation — presence and shape only, no §3–§11 semantics. It confirms: (a) all required instance-topic anchors **IT1–IT14** are present with stable `## §X.Y`-style headers; (b) the **§2 required instance header fields** are populated **and** the **export header diff summary** (previous version, new version, change category, IT sections touched) is present; (c) the **IT3 token list** carries columns `name / value / category / lifecycle-state / rationale-where-divergent`, the **IT4 component inventory** carries `name / tier / import-path / PC-variant / mobile-variant`, and the **IT11 breakpoint table** carries `name / range`; (d) the export SHOULD be scanned for obvious **feature-identifier patterns** (slug-like tokens, `Mn` milestones, `issue-` / `DR-` / `Q-` case-ids) in any IT section or the export header, and returned to CD if found — a heuristic catch for §1.4 reverse pointers; (e) the export SHOULD be scanned for **deprecated-token residue** — after any DSG §4.3 deprecate-with-successor, a token whose IT3 lifecycle state is `deprecated` MUST NOT appear as a **live binding** (a component state→token mapping, a composition / token-mapping / accessibility-posture binding, or a glossary "Binds …" line) anywhere except its own IT3/§3.2 deprecation/tombstone row; the deprecated token may still appear in deprecation-describing prose (overview, change-category, change-log). A deprecated token found used as a live binding is returned to CD — a heuristic catch for the §4.3 **migration-residue** class (a deprecate-with-successor leaves stale references when some consumer mappings or prose still name the old token after the successor rebinding). This check is *mechanical* — it needs no design judgment: it cannot judge §3–§11 semantic conformance, and the (d) feature-identifier scan and (e) deprecated-token-residue scan are pattern-matches, not full §1.4 / §4.3 conformance judgments (full §1.4 conformance is verified at the §15 review, item 17; full token-lifecycle conformance at §15 item 14). It runs as the first pass of the §12.3 review (today by Hub Claude in the review conversation; it could later be scripted). On any pre-check failure the export returns to CD **before** the semantic review and operator sign-off, so reviewer effort is spent on §3–§11 semantics rather than on detecting missing sections, malformed tables, obvious feature identifiers, or deprecated-token residue.

**Transfer mechanism**: The operator brings the CD-generated markdown into a Hub conversation for the §12.3 export conformance review; on a passing review, the reviewed export is committed to the CC monorepo per [MECH] Cross-Tool Workflow Handoff. The commit MAY be executed either by manual operator transfer or, as a transport modernization, via the **DesignSync MCP read-half** (`get_file` / `list_files` on the CD design-system project) under operator authorization — governance-equivalent, differing only in the pipe. The transport invariant is **review-then-fetch**: the §15 conformance review precedes and gates the transfer; the operator authorizes the fetch only after the review passes; the fetched bytes are the reviewed export (never fetch the project's live state in place of the reviewed artifact — that bypasses the gate). The transport choice does not alter §12.5 sync timing, the §15 review, or the read-only status of the CC mirror, and is **not** a direct CD↔CC coupling re-enablement (see [REF] Hub-CD-CC Architecture §9.4 / §10.5). The DesignSync **write-half is never used against the DS SOT** (§12.1 / §12.6).
...
## 13.3 UX-spec synthesis, on-demand CD re-entry, and export review

Neither Hub nor CC holds a DS instance copy. This section covers three consumption disciplines: UX Design Spec synthesis (firewalled CC session S1), the on-demand CD design-file re-entry path, and the export conformance review (Hub-side, meta-DS — unchanged).

- **UX Design Spec synthesis (firewalled CC session S1)**: UX Design Spec instances are synthesized in a CC session firewalled from the implementing session (S1, per [REF] Hub-CD-CC Architecture; the `ux-spec-synthesizer` role), at two scopes — phase-level (cross-cutting content) and per-feature (feature-scoped content). By **default** (app-level screens, no CD design file) synthesis grounds in the feature's PRD/TDD design intent and the DS inventory (via the CC mirror): every `component: <name>` reference resolves to a Tier A / Tier B entry already in the DS, and a needed component / token absent from the DS is a legitimate §12 additive proposal (captured as a per-feature UX Design Spec §2B.4 entry, additionally indexed in the phase-level instance §2A.6 if cross-cutting), not a defect. An independent reader (the `ux-spec-cross-checker` role) may verify the synthesis against its inputs on a **propose-not-author** basis (mirroring §12.1). CC additionally verifies references against the CC mirror at TK-04 M0 entry via SK-F, with CC code generation as the downstream backstop for any non-canonical reference.

- **On-demand CD design-file re-entry**: A CD design file is consumed **only** when a feature carries genuine **visual** novelty — operationally proxied by "needs a new design token / new visual language," NOT interaction / IA / component complexity (those are handled in CC plus meta-DS registration). A new-token signal auto-trips the trigger. CD is not a CC-invokable design service (the DesignSync MCP is pure file I/O — it cannot make CD generate), so the re-entry path is human-driven: ① CC pushes the feature's PRD/TDD text into the CD project's `uploads/` via the DesignSync write-half → ② **the operator designs in the CD UI (human-driven)** → ③ CC pulls the design file back via the read-half → ④ CC consumes it as a **visual reference** informing the UX Design Spec synthesis and the code. CC never authors design-output bytes into CD (that would make CC the producer — design output stays CD's), and never invokes CD to generate. DS-conformance of the design file is CD's responsibility (CD authored it holding the DS instance); a design file introducing a component / token not yet in the DS is a §12 additive proposal, not a defect.

- **Export conformance review**: When CD finalizes a DS instance change and generates a DS markdown export (§12.3 step 6), the operator brings the export into a Hub conversation. Hub Claude reviews the export against the §15 reviewer checklist to confirm the finalized instance conforms to §2-§11. This is the cross-workspace conformance gate — it catches divergence between the approved change plan and what CD authored, and any out-of-band CD change — before the export reaches the CC mirror. (This discipline is meta-DS and is unaffected by the app-level CD retire-default.)

These are consumption disciplines applied during the firewalled CC sessions and the Hub export-review conversation, not `.claude/skills/` artifacts; the synthesizing context holds DSG at runtime, so the disciplines track this rule automatically when it changes.
...
Both UX Design Spec instance types (phase-level + per-feature, synthesized in the firewalled CC session (S1) when any feature has `tier_1_involved=true` per `[TPL] UX Design Spec`) MUST reference the current DS instance version (the DS version the CC mirror carries at synthesis time, or the version recorded in a CD design file when one was pulled in on the on-demand re-entry path). Each per-feature instance declares the platform tier (per §11.2) the feature targets. If a feature introduces a new component, token, or layout pattern, the originating feature's per-feature UX Design Spec instance §2B.4 New-Components-Or-Tokens MUST include an update plan (per §12.4 change content structure); cross-cutting additives are additionally indexed in the phase-level instance §2A.6 (the index cross-references the originating feature's §2B.4 entry rather than duplicating the plan content). This triggers the §12 additive flow with CD-side authoring of the actual instance content change.

Both UX Design Spec instances are synthesized per the §13.3 consumption discipline — by default grounded in the feature's PRD/TDD design intent and the DS inventory (via the CC mirror), and grounded in a CD design file (as visual reference) only when one was pulled in on the on-demand re-entry path.

## 14.2 Pairing with intent.md UX brief

Each slice's intent.md MAY include a UX brief section listing screens and components used. The components referenced MUST exist in the instance §4 component inventory (Tier A or Tier B); platform coverage (PC, mobile, both) MUST be stated. The intent.md UX brief is authored in the firewalled CC session (S2, the relocated TK-03) by extracting from the slice-relevant subsets of both UX Design Spec instances — the per-feature instance (for in-slice UX content) and the phase-level instance (for cross-feature touchpoints / shared vocabulary the slice consumes) — both synthesized in the firewalled CC session (S1) per §13.3. The chain of grounding ensures slice-level UX brief content is DS-compliant.
...
Skills §13.1 and §13.2 consume the CC mirror of the DS instance plus this rule's content. The §13.3 disciplines (firewalled CC UX-spec synthesis, on-demand CD design-file re-entry, Hub-side export review) consume the DS inventory (via the CC mirror), a CD design file when one is pulled in on the on-demand path, and this rule's content. When this rule or the instance is updated (additive or breaking), the skills may need prompt adjustment and the §13.3 consumption disciplines track automatically. This is a paired-update relationship (P-19 in [OS] §8.5.2).
...
14. (If the update touches §4 tokens) token lifecycle states are valid — every deprecation names a live successor (Rule 4.3.2), no token is `removed` while a merged slice still references it (Rule 4.3.4), and any rename followed the add→deprecate→remove sequence (Rule 4.3.5); **and** no `deprecated` token is used as a **live binding** (component state→token mapping, composition / token-mapping / accessibility prose, or glossary "Binds …" line) anywhere except its own IT3/§3.2 deprecation/tombstone row — the deprecate-with-successor migration-residue check (**Rule 4.3.6**; mechanically pre-screened at §12.7(e))
...
- **UX Design Spec synthesis (S1) referencing a DS element absent from the DS inventory** — component / token / pattern claims in a synthesized UX Design Spec instance must resolve to a Tier A / Tier B entry already in the DS (via the CC mirror), or to a §12 additive proposal captured as a §2B.4 entry; a reference to a nonexistent or misnamed DS element that is neither in the inventory nor raised as an additive is drift per §13.3
- **A CD design file produced or consumed for an app-level screen without a genuine visual-novelty trigger** — the app-level default is CC-direct Arco-React with no CD design file; a design file is the on-demand re-entry path only, tripped by genuine visual novelty (proxied by a new design token / new visual language), never by interaction / IA / component complexity (those are CC + meta-DS registration). Routing ordinary app-level work back through a CD design file is drift toward the retired design-file-mandatory model per §13.3
- **CC authoring design-output bytes into CD, or "invoking" CD to generate** — on the on-demand re-entry path CC's only sanctioned CD write is delivering PRD/TDD text inputs into the CD project's `uploads/`; CC never writes design-output bytes (design output stays CD's) and the DesignSync MCP cannot make CD generate (pure file I/O). (Distinct from the meta-DS SOT write fence at §12.1 / §12.6, which this does not relax.)
...
- **A `deprecated` token still used as a live binding** — a token in lifecycle state `deprecated` after a DSG §4.3 deprecate-with-successor rebinding, still named as the active binding in a component state→token mapping, in composition / token-mapping / accessibility-posture prose, or in a glossary "Binds …" line, anywhere except its own deprecation/tombstone row — §4.3 **migration residue**: the successor rebinding (e.g. IN_REVIEW moved off a deprecated accent onto its `--hdc-color-*` successor) was applied to the authoritative binding but not fully propagated to the surrounding prose/glossary, leaving the instance internally self-contradictory. Mechanically pre-screened by the §12.7(e) export pre-check; confirmed at §15 item 14
