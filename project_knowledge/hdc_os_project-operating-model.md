...
| Cat 4 specification templates (TDD, Intent / Acceptance, Conversion Spec, Test Plan, Design System Governance, UX Design Spec, ADR Spec, Phase Test Plan) | ✗ | ✗ | ✗ | ✓ |
| Development Track `[RULE]` family (Claude Code Architecture Rules, Workspace Topology, Codex Plugin Usage) | ✗ | ✗ | ✗ | ✓ |
| Development Track `[MECH]` family (Development Track Workflow, CI/CD Milestone Policy, Application Lifecycle Handoff, Cross-Tool Workflow Handoff, Code Quality Rule Set, Dev-Loopback Mode, Tools Health Cadence) | ✗ | ✗ | ✗ | ✓ |
...
- Development Track inherits canonical constraints via CLAUDE.md files that inline-mirror canonical content (per §1.4 visibility boundary; CC cannot read hub canonical files at `/mnt/project/`). The inheritance scope currently includes:
  - [RULE] Claude Code Architecture Rules (tier boundaries, 10-agent roster, agent context scopes, repository layout, skill loading)
  - [RULE] Workspace Topology (multi-node infrastructure, branch topology, branch protection, walking-skeleton-first ordering, node assignment workflow, GitHub Issue marker block schema)
  - [RULE] Codex Plugin Usage (cross-model review and rescue, per-unit-type fire conditions, co-location semantics)
  - [MECH] Development Track Workflow (17-task orchestration, unit_type catalog, transition mechanism catalog)
  - [MECH] CI/CD Milestone Policy (M0–M5 gate semantics, per-unit-type milestone profile, evidence parity)
  - [MECH] Application Lifecycle Handoff (handoff readiness criteria, content scope, acknowledgment record format, re-entry policy)
  - [MECH] Code Quality Rule Set (tier-by-tier tool stack, severity policy, CI/CD pipeline integration, AI-era specific augmentations)
  - [MECH] Dev-Loopback Mode (single-command runnability contract, fixture content, placeholder implementation pattern, walking-skeleton M5 acceptance assertions)
...
|---|---|:---:|:---:|---|
| P-01 | [RULE] CCAR §6 ↔ [MECH] CI/CD §2 | SR | C2C | test-type taxonomy or tier-test assignment changes |
| P-03 | [MECH] DTW §4 ↔ [MECH] CI/CD §2 | SR | C2C | tasks added, renumbered, or re-scoped (milestone mapping re-verified) |
...
| P-09 | [MECH] DTW §4.0 ↔ [MECH] CI/CD §2.0 | SR | C2C | new unit_type added, milestone path modified, or scope variation revised |
| P-10 | [RULE] WT §6.2 marker schema ↔ [MECH] DTW §4.0 unit_type catalog | SR | C2C | catalog values change (new unit_type, value renamed, or unit_type retired) |
| P-11 | [TPL] TDD §3.Walking-Skeleton-Header + §3.Outputs ↔ [RULE] WT §4.6.2 + §4.6.3 | SR | C2C | walking-skeleton 6-output set changes or ordering rule's gate timing/applicability changes |
| P-12 | [MECH] CQ §3 ↔ [RULE] CCAR §1 | SR | C2C | tier semantics change (tier added, removed, or boundary modified) |
| P-13 | [MECH] Dev-Loopback §6 ↔ [MECH] CI/CD §2.6 | SR | C2C | M5 acceptance criteria or scope change |
| P-14 | [MECH] Dev-Loopback `apps/{app-slug}/dev/` ↔ [RULE] WT §4.6.3 | SR | C2C | WT changes Output #6 structure or enumerates `dev/` as sub-element |
| P-15 | [MECH] Dev-Loopback `apps/{app-slug}/dev/` ↔ [RULE] CCAR §Y.1 + §Y.2 | SR | C2C | CCAR changes app-scoped tree (path renaming or restructuring) |
| P-16 | [RULE] CCAR §X ↔ `.claude/config/context-scopes.yaml` | SR | C2R | scope allow/deny lists or agent assignments change |
| P-17 | [RULE] CCAR §5 + §5.3 ↔ `.claude/agents/{agent-name}.md` | SR | C2R | roster definition or scope assignment changes |
| P-18 | [RULE] CCAR §Z ↔ `.claude/skills/{skill-name}/SKILL.md` | SR | C2R | skill catalog or trigger conditions change |
...
| P-20 | [MECH] CQ §1–§4 ↔ lint and quality config files | SR | C2R | rule preset / architecture-lint pattern / severity policy / AI-era augmentation rule changes |
| P-21 | [MECH] Dev-Loopback §2–§5 + §7 ↔ `apps/{app-slug}/dev/**` + `HANDOFF.md` | SR | C2R | startup ceiling, fixture rules, placeholder pattern, ENV detection, or migration documentation changes |
...
| P-32 | [RULE] Codex §1.4 ↔ [MECH] DTW §4.0 + [MECH] CI/CD §2.0 | SP | C2C | unit_type catalog, milestone profile, or command-to-milestone anchors change |
| P-33 | [MECH] Application Lifecycle Handoff §5.2 ↔ [RULE] WT §4.6 + [TPL] TDD §3 | SP | C2C | WT §4.6 changes substantively (sub-section restructured, ordering rule modified, output set revised) or TDD §3 structure changes |
| P-34 | [MECH] CQ lint rules ↔ [RULE] DSG §4 + §5 | SP | C2C | DSG adds token, component, or changes consumption rules |
| P-37 | [MECH] Dev-Loopback §7 ↔ [MECH] Application Lifecycle Handoff §3.1 + §4.3 | SP | C2C | Handoff mandatory content list or §4.3 record format changes |
| P-38 | [MECH] Dev-Loopback §4 + §6 ↔ [MECH] DTW §4.0.2 + TK-04 / TK-05 / TK-08 / TK-13 | SP | C2C | walking-skeleton output set or affected TK semantics change |
| P-49 | [MECH] Tools Health Cadence §3 step 7 + §5.3 ↔ [MECH] CI/CD Milestone Policy §1.1 Claude Code baseline | SR | C2C | CI/CD §1.1 tooling baseline version pinning or upgrade verification procedure changes |
| P-50 | [MECH] Tools Health Cadence §5 P0 inventory + §3 step 6 ↔ [MECH] Code Quality Rule Set §1 / §2 tool stack + §8.5 Renovate Governance | SR | C2C | CQ §1 / §2 adds, removes, or substitutes a P0 tool; or CQ §8.5 Renovate config policy changes |
| P-51 | [MECH] Tools Health Cadence §3 step 8 + §5.4 ↔ [MECH] Dev-Loopback Mode §6 acceptance assertions + §2.3 readiness ceiling | SP | C2C | Dev-Loopback §6 assertion list or §2.3 ceiling changes |
| P-52 | [MECH] Tools Health Cadence §5 P0 inventory ↔ [RULE] Workspace Topology §3 tool stack per node | SR | C2C | WT §3 tool stack inventory adds, removes, or substitutes a tool |
| P-53 | [MECH] Dev-Loopback Mode §4.4 secret loading contract ↔ [MECH] Code Quality Rule Set §1.7 gitleaks + §1.2 hdc/no-inline-secret-literal + §4.6 secret leakage detection | SR | C2C | Either side changes: §4.4 mechanism revised, or CQ §1.7 / §1.2 / §4.6 rule set revised |
...
- **P-17** — Agent definition files in `.claude/agents/` are downstream deliverables, not canonical, but their content (purpose, frontmatter `tools` field, system prompt) must remain consistent with the §5.1 roster definition and §X context scope assignment. When the roster or scope assignment changes, affected agent files must update in the same revision; divergence breaks bias-firewall enforcement at runtime.
- **P-18** — Skill definition files in `.claude/skills/` are downstream deliverables, not canonical, but their content (load triggers, scope, override behavior) must remain consistent with the §Z.1 catalog and §Z.2 trigger conditions. When the catalog or trigger conditions change, affected SKILL.md files must update in the same revision; divergence breaks skill loading at runtime.
...
- **P-32** — The Codex §1.4 fire conditions table is derived from the conjunction of (a) DTW §4.0 unit_type catalog, (b) CI/CD §2.0 milestone profile, and (c) Codex §3.1 / §3.2 command-to-milestone anchors. When any of (a) / (b) / (c) changes, §1.4 must be re-verified for continued correctness.
...
- **P-34** — CQ encodes DSG token consumption (CSS-variable-only) and component import allow-list (Tier A Arco + Tier B HDC custom) as ESLint rules. When DSG adds a token, component, or changes consumption rules, the corresponding ESLint rule list must be re-verified within the same working period.
- **P-37** — Dev-Loopback §7 introduces `apps/{app-slug}/HANDOFF.md` (technical migration guide) extending Handoff §3.1 mandatory content; this artifact is distinct from the §4.3 `apps/{app-slug}/handoff-record.md` (governance acknowledgment record). When Handoff §3.1 mandatory content list or §4.3 record format changes, Dev-Loopback §7 documentation requirements must be re-verified within the same working period.
- **P-38** — Dev-Loopback artifacts are produced by TK-04 per DTW §4.0.2 as part of the walking_skeleton unit's output; consumed by TK-05 (M1 whitebox) and TK-08 (M2 contract / external integration); TK-13 (M5 staging deploy) gates on §6 acceptance assertions when the unit is `walking_skeleton`. When DTW §4.0.2 walking-skeleton output set or these TK semantics change, Dev-Loopback §4 / §6 references must be re-verified within the same working period.
- **P-49** — When [MECH] CI/CD Milestone Policy §1.1 tooling baseline changes (Claude Code version pinning, upgrade verification procedure, or supported version range), the [MECH] Tools Health Cadence §3 step 7 reference and §5.3 AI-dev infrastructure inventory must be re-verified within the same revision; divergence creates immediate sequencing inconsistency where the Tools Health Cadence step 7 baseline check fires against a stale baseline definition.
- **P-50** — When [MECH] Code Quality Rule Set §1 / §2 tool stack changes or when §8.5 Renovate Governance config policy changes, the [MECH] Tools Health Cadence §5 P0 inventory and §3 step 6 expectations must be re-verified within the same revision; divergence creates immediate semantic drift where the Tools Health Cadence inventory does not match the actual quality tooling.
- **P-51** — When [MECH] Dev-Loopback Mode §6 walking-skeleton M5 acceptance assertions or §2.3 readiness ceiling changes, the [MECH] Tools Health Cadence §3 step 8 verification expectation must be re-verified within the same working period.
- **P-52** — When [RULE] Workspace Topology §3 tool stack per node changes (tool added, removed, substituted, or version-policy changed), the [MECH] Tools Health Cadence §5 P0 inventory must be re-verified within the same revision.
- **P-53** — When [MECH] Dev-Loopback Mode §4.4 secret loading contract changes or when [MECH] Code Quality Rule Set §1.7 gitleaks / §1.2 hdc/no-inline-secret-literal / §4.6 secret leakage detection rule changes, both sides must be re-verified within the same revision; divergence creates immediate inconsistency where the positive-side secret-loading contract does not match the negative-side enforcement.
...
This section is a read view across the eleven cross-cutting Cat 4 sources from the `[RULE]` and `[MECH]` families that govern Claude Code execution architecture, infrastructure, orchestration, gating, tooling, and tool-stack maintenance. Each source's authoritative scope statement remains in its own §0 / §1 boundary chapter; this map exists so that readers can locate ownership across the family without grepping every source header.
...
| Source | Primary axis | Owns |
|---|---|---|
| `[RULE] Claude Code Architecture Rules` (CCAR) | Static structure | Tier boundaries (§1), subagent roster (§5), agent context scopes (§X), repository layout (§Y), skill loading rules (§Z), domain-package lifecycle (§Y.4), contract-testing convention (§Y.4) |
| `[RULE] Workspace Topology` (WT) | Multi-node infrastructure | Logical node identity (§2), node tool stack parity (§4), GitHub branch topology and workflow (§5 / §7), walking-skeleton-first ordering rule and 6-output canonical set (§4.6.2 / §4.6.3), node assignment workflow (§6), workspace inception (§10) |
| `[RULE] Codex Plugin Usage` | Cross-model review tool | Codex command semantics (§3), trigger logic (§1.4), evidence path schema (§2), review-gate defaults (§4), per-unit-type fire conditions table (§1.4) |
| `[MECH] Development Track Workflow` (DTW) | Task-level orchestration | TK-01 through TK-13 sequence (§4), unit_type catalog (§4.0), transition mechanisms (§5), human intervention points (§6), failure routing matrix (§7) |
| `[MECH] CI/CD Milestone Policy` | Lifecycle gating | M0–M5 gate semantics (§2), per-unit-type milestone profile (§2.0), evidence parity (§1.2), slice-size advisory (§2.7), M5 single-staging-deploy design (§2.6), Test Evidence Report contract (§6) |
| `[MECH] Application Lifecycle Handoff` | App-to-human-team transition | Application lifecycle stages (§1), handoff readiness criteria (§2), content scope (§3), mechanism (§4), tag namespace (§4.1), re-entry policy (§5) |
| `[MECH] Cross-Tool Workflow Handoff` | Cross-tool content flow contracts | Three operator-mediated handoff paths (Hub ↔ CD, Hub ↔ CC, CD ↔ CC), per-direction content contracts (§2-§4), reminder-form discipline (§5), audit-failure handling (§6), Hub Claude trigger phrases for cross-tool handoff (§7), CD ↔ CC decoupled-by-default discipline during research preview (§4.4) |
| `[MECH] Code Quality Rule Set` (CQ) | Code quality enforcement | Tool stack and preset chain (§1), rule presets (§2), custom architecture lint rules (§3), AI-era augmentations (§4), severity policy (§5), CI integration (§5 milestone fire mapping) |
| `[MECH] Dev-Loopback Mode` | Single-machine runnability | Single-command startup (§2), fixture content rules (§3), placeholder pattern with Tier A / Tier B classification (§4), ENV switch gate (§5), walking-skeleton M5 acceptance assertions (§6), HANDOFF.md migration document (§7) |
| `[MECH] Tools Health Cadence` | Tool-stack maintenance | Trigger model (§2), Hub-Claude-executable eleven-step execution protocol (§3), action-item prioritization scheme (§3.1), quarterly tools health report structure (§4), P0 tool stack inventory across all tiers (§5), Hub Claude soft compliance trigger phrases (§6) |
| `[RULE] Design System Governance` (DSG) | Tier 1 design language governance | Singleton instance model (§1), instance section contract (§2), implementation path rules (§3), token governance (§4), component governance (§5), accessibility stance (§6), i18n & RTL governance (§7), motion hygiene (§8), iconography rules (§9), content style governance (§10), platform tier framework (§11), update flow (§12), custom skill integration (§13), pairing rules (§14), reviewer checklist (§15), anti-drift red flags (§16) |

**Ownership rule**: Any topic listed in one source's "Owns" column is canonically owned by that source. Other Cat 4 sources reference but do not redefine. When a topic's owner is genuinely ambiguous (operationally relevant to multiple sources), surface as anti-drift signal per §12 rather than dual-owning.
...
- `rule` — format, tool, and architecture rules. Format rules govern output discipline (e.g., DingTalk Markdown specification). Tool rules govern tool/platform behavior (e.g., Claude Platform Behavior, Codex Plugin Usage). Architecture rules govern code or infrastructure structure (e.g., Claude Code Architecture, Workspace Topology, Design System Governance).
...
Properties:
- **Authoring**: Hub-authored — Hub Claude drafts content based on canonical [RULE] CCAR §5 / §Z; operator reviews and transfers into Development Track via operator-mediated handoff per [MECH] Cross-Tool Workflow Handoff §3.1.
...
| `[RULE]` (tool-focused) | `Tool Usage Specification` | e.g., Claude Platform Behavior, Codex Plugin Usage |
...
| `[MECH]` (governance-focused) | `Governance Mechanism Specification` | e.g., CI/CD Milestone Policy, Code Quality Rule Set, Dev-Loopback Mode |
