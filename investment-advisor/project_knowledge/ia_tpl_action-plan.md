...
- **Pre-band stage (观察线).** A locked finalist enters the plan with either a derived band (Act-now/Armed) or — if not yet in a valuation-justified domain — a **watch-alert line**: a cheap pre-gate trigger carrying no target zone, funding, or size. Crossing it promotes the name to the §6.7 derivation above; a watch-alert cross is never an auto Act-now.
...
- **Watch-alert line (观察线)** — a locked finalist (§5.3 Shortlist) **not yet held and with no §6.7 band derived**: an advisor-set price threshold that, when crossed, **triggers a full §6.7 derivation** — valuation gate → if confirmed, the name gains an Armed band; if not, an open thread. It is a *look-trigger, not an executable action*: state "on cross, derive & re-assess," never a size, funding, or "buy at ¥X." Distinct from Armed, which already carries a derived zone.
...
5. **Watch-alert-line completeness.** Every locked-but-unheld finalist carries either an Armed/Act-now band OR a watch-alert line; a finalist with neither is a plan-maintenance gap — surface it (缺观察线), never silently skip.
6. **No personal data in the public repo.** The filled plan (real instruments, sizes, capital) is private; only this blank schema is committed.
