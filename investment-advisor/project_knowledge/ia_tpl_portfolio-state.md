...
**Canonical schema (the blank `.env.example`) for the Investment Advisor's portfolio-state runtime data.** Carries zero real data — only field definitions and illustrative examples. To use: copy this into your private filled-data location (outside this public repo), delete the examples, enter real positions, and upload **only the filled copy** to claude.ai project knowledge. Never enter real positions/capital into this canonical copy. The advisor reads the *filled instance* per Investment Strategy Framework §10; you maintain **shares + cost basis + thesis status**, and the advisor fills **current price / market value / current %** at review (via web lookup, never memory). Refresh the filled snapshot at each monthly (light) and quarterly (deep) review.

**Two update modes (framework §10) — re-price vs re-snapshot.** *Re-price (mark-to-market):* between trades, with **shares + cost basis held fixed**, the advisor refreshes price / market value / % / unrealized P&L / valuation percentile from the live data source (incl. **dividends, 送转/拆股, FX** where available); automatic and frequent, accuracy bounded by the source. *Re-snapshot (authoritative):* after a **share-count change** (trade, 送转/拆股) or **account-level cash flow** (transfer, repo interest, fees, settlement timing), supply the broker snapshot — a screenshot is fine, and dropping it is the trigger; the advisor **verifies the parse by reconciliation, auto-commits what reconciles, and flags only what doesn't — the gate is verification, not human sign-off** (framework §10), then reconciles against the Transaction Log. Mind each artifact's domain: a **holdings snapshot** sets static State; a **成交回单** is the fills / trade-path source and **overrides any path earlier inferred from a State delta** (an overwrite, not a pending-confirmation item); **cash-flow events (transfers, dividends) need a 资金流水/对账单**, which neither screenshot supplies. Re-pricing never invents a trade or silently absorbs a share/cash event; the source-blind inputs (your actual trades, account-level cash flows, dividend tax-net) need your input. Equity-vesting is the §9.3 human-capital input, **not** a brokerage share-count event.
