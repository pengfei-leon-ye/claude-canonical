...
| Cross-node Codex invocation anti-pattern | [RULE] WT + [RULE] Codex (co-owned) | WT §3.5 + §7; Codex §8 | DTW §8 (TK-04 / TK-13 trigger-time view) |
...
| Codex-tool-specific (invocation pattern drift; path drift; M0 review on partial spec bundle; rescue used repeatedly for same problem; cross-app Codex invocation) | [RULE] Codex Plugin Usage | §8 | (none) |
...
| Source-authoring neutrality | [MECH] Dev-Loopback Mode | §8 | Different kind of "anti-drift" — concerns authoring this source itself (brand neutrality, spec reference neutrality, milestone semantics neutrality, repository layout reference neutrality), not runtime operational red flags |
...
- Runtime detection mechanism (CI tooling, lint rules, agent invocation hooks — see [MECH] Code Quality Rule Set §5 for deterministic enforcement, [MECH] CI/CD Milestone Policy for milestone-gate enforcement)
