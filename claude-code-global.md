...
- Capacity — **estimate window utilization, then trigger on the estimate** (the model cannot read the window %, and asking the operator each checkpoint is too slow). Compute a running estimate at each checkpoint:
  ...
  - `OVERHEAD ≈ 65K tokens` — session-start fixed cost (system prompt + tool schemas + skills / MCP instructions + the CLAUDE.md / MEMORY.md set). Floor ~10K is measured from the canonical files; the system+tools portion (~40–70K) is estimated, not model-readable — treat 65K as a working constant and let the compaction cross-check catch gross error.
