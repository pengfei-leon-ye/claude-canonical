...
- **GitHub branch/tag protection — two-tier, active since 2026-06-15.** The personal repos carry protection matched to their workflow:
  ...
  The live ruleset on GitHub is the source of truth; this records the rationale so the model is not misread or "corrected" against its own logic.
...
- Capacity — task-typed two-track **proxy** ladder. The proxy formulas are what I actually trigger on; the parenthetical % anchors describe the operator-side truth signal the proxy is estimating, and apply only when the operator shares the CC-client value.
  ...
  - **Rationale**: Anthropic does not publish hard thresholds — degradation is gradual and task-dependent. Community heuristics center on ~60% utilization for general intervention [网检·中·3独立源·2026]; linear tasks tolerate noticeably higher utilization than multi-hop / random-access ones. Lost-in-the-middle persists but is materially less pronounced in current frontier models than in earlier ones [网检·中·3独立源·2025]. The two-track ladder reflects this — do not collapse it back into a single number, and do not present operator-side % numbers as if they were my-side observables.
